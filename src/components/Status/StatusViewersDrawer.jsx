...
export function StatusViewersDrawer({
  isOpen,
  onClose,
  views = [],
  activeStatus = null,
  ...
  // Reset limit when drawer opens or closes or status changes
  React.useEffect(() => {
    if (!isOpen) {
      setVisibleLimit(10);
    }
  ...
  const chronologicalViews = [...views].sort(
    (a, b) => new Date(a.createdAt) - new Date(b.createdAt),
  );
  const firstViewerId = chronologicalViews[0]?.viewerId;
  const lastViewerId =
    chronologicalViews[chronologicalViews.length - 1]?.viewerId;

  const sortedViews = [...views].sort(
    (a, b) => new Date(b.createdAt) - new Date(a.createdAt),
  );
  const renderedViews = sortedViews.slice(0, visibleLimit);
        ...
        >
          ...
          >
            {renderedViews.length > 0 ? (
              renderedViews.map((vw, index) => {
                const isFirst =
                  vw.viewerId === firstViewerId && views.length > 1;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 max-w-[50%]">
                      ...
                      {vw.questionAnswer && (
                        <span
                          className="text-xs bg-[#7f66ff]/10 text-[#a899ff] border border-[#7f66ff]/20 rounded-lg px-2.5 py-1 truncate max-w-[120px]"
                          title={vw.questionAnswer}
                        >
                          💬 "{vw.questionAnswer}"
                        </span>
                      )}
                      {vw.reaction && (
                        <span
                          className="text-lg bg-[#2a3942] rounded-full p-1 leading-none shadow"
                          title={t("chat.react") || "Reaction"}
                        >
                          {vw.reaction}
                        </span>
                      )}
                      {vw.replyText && (
                        <span
                          className="text-xs bg-[#2a3942] text-white/90 rounded px-2.5 py-1 truncate max-w-[120px]"
                          title={vw.replyText}
                        >
                          💬 {vw.replyText}
                        </span>
              ...
              <div className="text-center py-8 text-xs text-white/40">
                {t("status.no_views_desc") ||
                  "No views yet. Share updates to see viewers here."}
              </div>
            )}
          </div>
        </motion.div>
